...
C_UR::C_UR(std::string hostName, unsigned int portNum, double _delta_t) : v(6), delta_t(_delta_t), rtde_control(hostName),rtde_receive(hostName)
{
...
//    osChaine << "speedl([" << v[0] << ", " << v[1] << ", " << v[2] << ", " << v[3] << ", " << v[4] << ", " << v[5] << "], a=2.0, t=0.05)\n";//, t=" << delta_t << ")\n";
    osChaine << std::fixed << std::setprecision( 4 ) << "speedl([" << bv[0] << ", " << bv[1] << ", " << bv[2] << ", " << bv[3] << ", " << bv[4] << ", " << bv[5] << "], a=1, t=0.05)\n";//, t=" << delta_t << ")\n";
...
int C_UR::getCameraArticularPose(vpColVector &j)
{
 	 j = rtde_receive.getActualQ();
