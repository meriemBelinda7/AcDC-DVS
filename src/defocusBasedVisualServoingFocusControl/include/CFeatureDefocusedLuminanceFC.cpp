/*!
  \class CFeatureDefocusedLuminanceFC.cpp
...
#include "CFeatureDefocusedLuminanceFC.h"

void CFeatureDefocusedLuminanceFC::init(unsigned int _nbr, unsigned int _nbc, double _Z)
{
...
*/
CFeatureDefocusedLuminanceFC::CFeatureDefocusedLuminanceFC() : vpFeatureLuminance(), pixInfo(NULL),  cam(8e-3,5e-6,1,0.25,320,240)
{
  nbParameters = 1;
  dim_s = 0;
  flags = NULL;
...
/*!
 Copy constructor.
 */
CFeatureDefocusedLuminanceFC::CFeatureDefocusedLuminanceFC(const CFeatureDefocusedLuminanceFC &f)
...
/*!
 Copy operator.
 */
CFeatureDefocusedLuminanceFC &CFeatureDefocusedLuminanceFC::operator=(const CFeatureDefocusedLuminanceFC &f)
{
  Z = f.Z;
  nbr = f.nbr;
  nbc = f.nbc;
...
/*! 
  Default destructor.
*/
CFeatureDefocusedLuminanceFC::~CFeatureDefocusedLuminanceFC() 
{
  if (pixInfo != NULL)
  {
    delete [] pixInfo ; 
...
void
CFeatureDefocusedLuminanceFC::setCameraParameters(CCameraThinLensParameters &_cam) 
{
  cam = _cam ;
}
...
*/
void
CFeatureDefocusedLuminanceFC::buildFrom(vpImage<unsigned char> &I)
{
	unsigned int l = 0;
  double Ix, Iy;
	double Ixx;
...
/*
  Returns the number of controlled degrees of freedom
*/
/*!
...
*/
void
CFeatureDefocusedLuminanceFC::interaction(vpMatrix &L)
{	
  L.resize(dim_s, 7);
	double f = cam.get_f();
  ...
  for (unsigned int m = 0; m < L.getRows(); m++) {
    ...
    double IxxBetailZ = -Ixx*(cam.get_D()*f*(Z - f))/(6*(cam.get_ku()*Z*(pow((cam.get_Zf()-f),2))));
...
int CFeatureDefocusedLuminanceFC::getDim()
{
  return dim_s;
};
/*!
...
*/
vpMatrix CFeatureDefocusedLuminanceFC::interaction(const unsigned int /* select */)
{
...
{
  CFeatureDefocusedLuminanceFC *feature = new CFeatureDefocusedLuminanceFC ;
  return feature ;
}
